...
Header-only heap / stack / reset-cause health monitor for ESP32 boards running
the Arduino-ESP32 framework (core 2.x and 3.x). Transport-agnostic: it produces
...
```ini
lib_deps =
    https://github.com/rfanfant/esp32-sys-health.git#v1.0.0
```
...
- The header holds module state in `static` variables and an `RTC_NOINIT_ATTR`
